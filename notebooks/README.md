...
unresolved author questions are in the [legacy workspace inventory](../docs/legacy_workspaces_inventory.md).
The earlier [whole-volume ROI audit](../docs/platyneris_data_inventory.md) remains a useful crop-extraction
...
## Checkpoint contracts and configuration

Notebook 02 uses the `position-aware-3d-v2` crop MAE contract. Notebook 04 uses
`grouped-nucleus-patches-v3`, whose tokens are complete stored patches from one
nucleus. Keep checkpoints paired with their matching architecture and input
contract; older unversioned and single-patch checkpoints cannot resume grouped
training.

Notebook 04 uses `configs/sites/mae_platynereis_nuclei_embl.yaml` when its external
paths are mounted. Otherwise it resolves `configs/mae_nucleus_patches_template.yaml`
from `MORPHOFEATURES_NUCLEUS_PATCHES_N5` and `MORPHOFEATURES_NUCLEUS_POSITIONS_N5`.
Without either source, real-data cells skip explicitly.

## Maintaining the notebooks

The narrative cell sources for notebooks 01 and 02 live in
`build_tutorial_notebooks.py`. Notebook 04 is canonical in notebook form;
`build_real_mae_notebook.py` validates it and clears execution state. The tutorial
builder must preserve its canonical cells rather than carry a second narrative.
